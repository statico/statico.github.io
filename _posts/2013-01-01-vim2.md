...
Finally, everyone knows about <kbd>Ctrl-N</kbd> and <kbd>Ctrl-P</kbd> for basic completion. But did you know about <kbd>Ctrl-X</kbd><kbd>Ctrl-L</kbd> for whole-line completion? Give it a try.
...
Vundle uses the service at [vim-scripts.org](http://vim-scripts.org/) (which I had never seen before) to find and install packages into `~/.vim/bundle/`, which seems great. It even has a search mechanism and interactive installer.

However, Vundle is still very new software. I tried replacing my own management system with Vundle, but I did not have a good experience. I was a little alarmed at first since, to my knowledge, the [ancient Vim script registry](http://www.vim.org/scripts/index.php) has no index or API, and I was curious what would be installed when I requested to install `Zenburn`. It was only after reading the source that I discovered the new [vim-scripts.org](http://vim-scripts.org/) site.

After adding `Bundle` statements to my `vimrc`, running the `vim +BundleInstall +qall` command dropped me into its weird interactive installer. It took me a minute to realize I was supposed to press <kbd>i</kbd> on each line to install the module, and then trying to install half of the modules from GitHub resulted in GitHub asking me for a username and password, thus an error.

I eventually installed everything, or so I think, and things seemed to work, but I wasn't reassured. Vundle is promising, but it's got a ways to go before I can call it a solid package manager that's on the tier of cpan, gem, apt, npm or pip. Until then I'll stick with my trusty, comprehensible, Pathogen-based management system and [`update.sh` script][updatesh].
